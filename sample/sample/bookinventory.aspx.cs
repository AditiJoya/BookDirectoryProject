	...
	public partial class bookinventory : System.Web.UI.Page
	{
		...
		void updateBookByID()
		{
			...
			if (checkIfBookExists())
			{
				try
				{
					...
					cmd.Parameters.AddWithValue("@book_buy_link", TextBox12.Text.Trim());
		...
		void getBookByID()
		{
			try
			{
				...
				if (dt.Rows.Count >= 1)
				{
					...
					TextBox12.Text = dt.Rows[0]["book_buy_link"].ToString();
		...
		void addNewBook()
		{
			try
			{
				...
				string filepath = "~/book_inventory/b3.jpg";
				string filename = Path.GetFileName(FileUpload1.PostedFile.FileName);
				FileUpload1.SaveAs(Server.MapPath("book_inventory/" + filename));
				filepath = "~/book_inventory/" + filename;
				...
				SqlCommand cmd = new SqlCommand("INSERT INTO book_master_table(book_id,book_name,genre,author_name,publisher_name,publisher_date,language,edition,book_cost,no_of_pages,book_description,actual_stock,current_stock,book_img_link,book_buy_link) values(@book_id,@book_name,@genre,@author_name,@publisher_name,@publisher_date,@language,@edition,@book_cost,@no_of_pages,@book_description,@actual_stock,@current_stock,@book_img_link,@book_buy_link)", con);
